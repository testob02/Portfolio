...
st.set_page_config(layout='wide',page_title="VeggieView",page_icon=":carrot:")
st.markdown(
"""
<style> 
.stDeployButton {
...
def load_model():
    ...
    Returns:
        model: a keras model
    '''
    model = tf.keras.models.load_model('Fruits and Vegetables/model.keras')
    return model
...
def get_classes():
    ...
    '''
    with open('Fruits and Vegetables/labels.txt', 'r') as f:
...
st.title(':blue[VeggieView :carrot:]')
st.write(":blue[Explore the world of fresh produce with VeggieView]")
st.write(":blue[Your go-to app for identifying all things fruits and vegetables]")
...
if uploaded_image:
    ...
    with st.spinner(':blue[Please be patient as our digital gardeners analyze the details :broccoli:]'):
